...
import HistoryContext from '../../component/common/history/context/context'
...
const GetColumnSearchProps = (titles: any[]) => {
    ...
    const onFinish = (values: any) => {
        ...
        if (values?.filters != undefined && values?.filters.length > 0) {
            values?.filters?.map((filter: any) => {
                let found = titles.find(title => filter.title === title.name);
                let value = null;
            ...
            historyContext.onResubmitFilter(true);
            historyContext.onSetSubmittedFilters(filtersWithOperand);
            historyContext.onSetIsAdvancedSearch(true);
            historyContext.searchAgain();
            historyContext.onChangeActiveCollapsiblePanel();
        }
    ...
    const removeAllFilters = () => {
        ...
        historyContext.resetFilterValues()
        historyContext.onResubmitFilter(true);
        historyContext.searchAgain();
        historyContext.onSetIsAdvancedSearch(false);
        historyContext.onChangeActiveCollapsiblePanel();
    }
                            ...
                            {(fields, { add, remove }) => (
                                <>
                                    {fields.map(({ key, name, fieldKey, ...restField }) => (
                                        ...
                                        <Row>
                                            <Col xs={8} sm={8} lg={8} xl={8} className="p-l-8">
                                                <Form.Item
                                                    {...restField}
                                                    name={[name, 'title']}
                                            ...
                                            <Col xs={6} sm={6} lg={6} xl={6} className="p-l-8">
                                                <Form.Item
                                                    {...restField}
