...
function SignatureModalHook(props: any) {
	...
	useEffect(() => {
		if (props.isUpdate) {
			...
			SignatureApi.GetFirstSignatureById(props.signatureId).then((response: any) => {
	...
	const onFinish = (values: any) => {
		...
		if (props.isUpdate) {
			...
			SignatureApi.UpdateFirstSignature(signatureEdit).then((response: any) => {
			...
			SignatureApi.InsertFirstSignature(signatureAdd).then((response: any) => {
				setIsLoadingSubmitBtn(false);
				if (response?.data?.IsSucceed) {
					Notification.success({ message: ' با موفقیت ثبت شد.' });
					props.onCloseAddEditMoadl();
